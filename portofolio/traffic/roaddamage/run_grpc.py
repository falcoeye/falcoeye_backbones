...
fileName = "./examples/roaddamage.png"
...
response =  stub.Predictions(
    PredictionsRequest(model_name="roaddamage-4m",
                                    input=input_data))
prediction = response.prediction
print("to literal eval")
detections = np.frombuffer(prediction,dtype=np.float32).reshape((-1,6))
bboxes = detections[..., :4].reshape(-1,4)
scores = detections[..., 4].reshape(-1,1)
classes = detections[..., 5].reshape(-1,1)
print(bboxes)
print(scores)
print(classes.min(),classes.max(),scores.min(),scores.max())