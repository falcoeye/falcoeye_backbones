...
def get_args_parser():
    ...
    parser.add_argument(
        '--img', metavar='IMG',default="./denmark-litter.jpeg",
    ...
    parser.add_argument(
        '--checkpoint', type=str,default="./detect-waste.pth",
        help='path to checkpoint')
    parser.add_argument(
        '--device', type=str, default='cpu',
        help='device to evaluate model (default: cpu)')
    parser.add_argument(
        '--prob_threshold', type=float, default=0.1,
...
def main(args):
    ...
    num_classes = len(args.classes)
    model_name = "efficientdet_d2"#args.checkpoint.split('-')[-1].split('/')[0]
    model = set_model(model_name, num_classes, args.checkpoint, args.device)
    ...
    print(type(outputs))
    # keep only predictions above set confidence
    mask = outputs[0, :, 4] > args.prob_threshold
    bboxes_keep = outputs[0, mask]
    probas = bboxes_keep[:, 4:]
